...
pub fn layout_flex(
    ...
    let geoms: Vec<ItemGeom> = if is_row {
        ...
        items.iter().enumerate().map(|(idx, item)| {
            ...
            let cross = if let Node::Element(e) = item {
                ...
                let h = measure_block_children(ls, fonts, e,
                    inner_x + content_w.max(1), is);
                ls.cursor_x    = old_cx;
                ls.cursor_y    = old_cy;
                ls.margin_left = old_ml;
                ls.indent      = old_indent;
    ...
    let cross_inner = if is_row {
        container_h - pad_t - pad_b
    } else {
        inner_w
    };
    ...
    for (slot, &idx) in indices.iter().enumerate() {
        ...
        let physical_main = if is_reverse {
            inner_main - main_cursor - item_main
        } else {
            main_cursor
        };
        ...
        let pre_y = child_y - is.margin.top;
        let pre_x = child_x - is.margin.left;

        ls.cursor_x    = pre_x;
        ls.cursor_y    = pre_y;
        ls.margin_left = pre_x;
        ls.indent      = 0;
        ls.line_height = is.font_size as i32;
